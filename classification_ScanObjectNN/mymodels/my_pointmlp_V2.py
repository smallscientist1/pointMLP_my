...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    Input:
        ...
        nsample: sample number in local region(if there are less points in the local region,repeat first points)
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            # according to the paper, not exactly std
            # std = torch.norm((grouped_points-mean).reshape(B,-1),p=2,dim=-1,keepdim=True).unsqueeze(dim=-1).unsqueeze(dim=-1)
            # std = std/((grouped_points.shape[1]*grouped_points.shape[2]*grouped_points.shape[3])**0.5)
...
class ConvBNReLU1D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, bias=True, activation='relu'):
        ...
        self.net = nn.Sequential(
            ...
            self.act if activation is not None else nn.Identity()
        )
...
class PosExtractionV2(nn.Module):
    ...
    def forward(self, xyz,x):  # [b,g,3],[b, d, g]
        x2 = self.group(xyz,x.permute(0,2,1))  # [b,d,g]
        x2 = self.MLP2(x2)
        x2 = self.MLP3(x2)
        x = F.relu(x+x2)
        return x
...
class LocalAggregation(nn.Module):
    def __init__(self, channel, kneighbors, scale_up_idx, use_xyz=True, normalize="anchor", **kwargs):
        ...
        self.radius = 0.15 # 0.15 for ScanObjectNN
        self.radius_scale = 1.5
        self.nsample = 32
        self.radius = self.radius * (self.radius_scale**scale_up_idx)

        add_channel=3 if self.use_xyz else 0
        self.conv = ConvBNReLU1D(channel+add_channel, channel, bias=False, activation='relu')
    ...
    def forward(self, xyz, points):
        ...
        grouped_points = index_points(points, idx)  # [B, npoint, k, d]
        if self.use_xyz:
            # grouped_points = torch.cat([grouped_points, grouped_xyz-xyz.view(B,N,1,-1).repeat(1,1,self.kneighbors,1)],dim=-1)  # [B, npoint, k, d+3]
             grouped_points = torch.cat([grouped_points, (grouped_xyz-xyz.view(B,N,1,-1).repeat(1,1,self.nsample,1))/self.radius],dim=-1)

        new_points = grouped_points.permute(0,1,3,2) # [B, npoint,d+3, k]
        b, npoint, d, k = new_points.shape
        f = new_points.reshape(-1,d,k) # [B*npoint, d+3, k]
        f = self.conv(f) # [B*npoint, d, k]
        f = F.adaptive_max_pool1d(f, 1) # [B*npoint, d, 1]
        f = f.reshape(b,npoint,-1).permute(0,2,1) # [B, d, npoint]
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        for i in range(len(pre_blocks)):
            ...
            pos_block_module = PosExtractionV2(out_channel,i+1, pos_block_num, groups=groups,
                                                res_expansion=res_expansion, bias=bias, activation=activation,kneighbors=kneighbor)
